...
\newcommand{\Summ}[0]{\sum_{i=1}^m}

\newcommand{\Prodn}[0]{\prod_{i=1}^n}

\newcommand{\Prodm}[0]{\prod_{i=1}^m}

\newcommand{\MLE}[1]{\widehat{#1}_\text{MLE}}

\newcommand{\MME}[1]{\widehat{#1}_\text{MME}}
...
\newcommand{\Bin}[1]{\text{Binomial}\left(#1\right)}

\newcommand{\Bern}[1]{\text{Bernulli}\left(#1\right)}

\newcommand{\Gamd}[1]{\text{Gamma}\left(#1\right)}

\newcommand{\Betad}[1]{\text{Beta}\left(#1\right)}

\newcommand{\Geo}[1]{\text{Geometric}\left(#1\right)}
...
#function which parittions the data into a number of groups (input), calculates apeglm estimates for the groups with C++, and outputs elapse time for MLE, overdispersion and apeglm estimation
...
iWantToBeDoneWithThisPaperAlready = function(ls) {
  mean(ls$time)
}
meanAodTimes = unlist(lapply(timesAod, iWantToBeDoneWithThisPaperAlready))
meanGamlssTimes = unlist(lapply(timesGamlss, iWantToBeDoneWithThisPaperAlready))
meanHrqTimes = unlist(lapply(timesHrq, iWantToBeDoneWithThisPaperAlready))
meanAods3Times = unlist(lapply(timesAods3, iWantToBeDoneWithThisPaperAlready))
meanVgamTimes = unlist(lapply(timesVgam, iWantToBeDoneWithThisPaperAlready))
meanAodTimes/100000
...
## microbenchmark plot
```{r}
meanAodTimes = unlist(lapply(timesAod, iWantToBeDoneWithThisPaperAlready))[2:10]/1000000
meanGamlssTimes = unlist(lapply(timesGamlss, iWantToBeDoneWithThisPaperAlready))[2:10]/1000000
meanHrqTimes = unlist(lapply(timesHrq, iWantToBeDoneWithThisPaperAlready))[2:10]/1000000
meanAods3Times = unlist(lapply(timesAods3, iWantToBeDoneWithThisPaperAlready))[2:10]/1000000
meanVgamTimes = unlist(lapply(timesVgam, iWantToBeDoneWithThisPaperAlready))[2:10]/1000000
meanProregTimes = unlist(lapply(timesProreg, iWantToBeDoneWithThisPaperAlready))[2:10]/1000000
meanApeglmTimes = unlist(lapply(timesApeglm, iWantToBeDoneWithThisPaperAlready))[2:10]/1000000
...
```{r}
ggplot(data = timesMicro, mapping = aes(y=times, x=rep(c(2:10), 6), color=Package, fill=Package)) +
  ...
  scale_color_manual(values = dichromat(c("#000000","#CDCD00","#D55E00","#0000FF", "#FF69B4", "#56B4E9"))) +
  scale_fill_manual(values = dichromat(c("#000000","#CDCD00","#D55E00","#0000FF", "#FF69B4", "#56B4E9"))) +
...
## Comparisons of estimates and their associated log-likelihoods 
There does appear to be noteworthy differences between apeglm and aod and many instances of convergence failures for aod, but they are not neccesarily related. C++ estimates are always closer to VGAM and have a higher log-likelihood (even for genes where converence occurs for all method).

```{r}
cat("coefficient estimates and associated log-likelihoods for apeglm, VGAM and ash (in that order)")
i=2
j=636
...
## Benchmarking Aod and VGAM Package

Below gives elapsed time for ML estimation with apeglm and with aod. Apeglm was run with two iterations of overdispersion estimation. We can see that our package is substantially faster, and the difference grows the more groups are involved. Furthermore, aod frequently had converge issues (maximum iterations reached), while all of our estimates converged. Our estimates differed from aod, but between the convergence warnings issued and the fact that our estimates were much closer to VGAM than aod for one of the genes chosen at random, we conclude that it is our estimates that are correct. 
...
data.frame(2:10) %>%
  cbind(timesA) %>%
  cbind(timesMC) %>%
  rename(groups = X2.10, aodTime = timesA, apeglmTime = timesMC)
...
```{r, echo=F}
timesC = c(30.946,34.473,37.165,39.451,42.144,44.436,47.603,48.939,51.531)
timesO = c(156.803, 262.406,  361.673,  511.782,  645.705,  813.858,  985.942, 1195.837, 1414.874)
timesMC = c(23.900, 25.146, 27.278, 28.268, 29.690, 31.106, 32.879, 34.297, 35.497)
timesA = c(165.787,  284.827,  399.067,  501.456,  643.628,  761.160,  869.816, 1014.554, 1176.624)
