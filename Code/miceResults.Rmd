...
Unlike in the simulations, everything looks almost exactly the same as when lower tolerance was used. However, recall that the holdover MLE is being taken as the truth, and thus MLE biases that effect the estimates will affect the "truth" in the same way. 
...
#    }
# saveRDS(results, "~/Desktop/crossValResultsSeed2395-size5.rds")
...
#   cat("\n\n\n\n\n")
# }
# saveRDS(CIashexact, "~/Desktop/CIashexact.rds")
...
#write.csv(map, "~/Desktop/caseMAP.csv")
#write.csv(subres$map$sd, "~/Desktop/caseMAPSD.csv")
```
...
MAplots = cowplot::plot_grid(MAplot1, MAplot2, align = "hv", ncol=1) +
  ggsave("~/Desktop/ResultsLove/PaperPlots/MAplotsReal.png")
plot(MAplots)
```
...
MAplots = cowplot::plot_grid(MAplot1, MAplot2, align = "hv", ncol=1) +
  ggsave("~/Desktop/ResultsLove/PaperPlots/MAplotsRealColorblinded.png")
plot(MAplots)
```
...
ggsave("~/Desktop/ResultsLove/PaperPlots/realFigure5.png", fig)
```
...
grid.arrange(tveMle, tveApeglm, tveAsh, catPlot)
#fig = arrangeGrob(tveMle, tveApeglm, tveAsh, catPlot)
#ggsave("~/Desktop/ResultsLove/PaperPlots/realFigure.png", fig)
```

```{r}
cond1 = shrink>pmax(.25,mle/4)
cond2 = shrinkash>pmax(.25,mle/4)
cond3 = (cond1 & cond2) | (cond1&!cond2)
tveMle = ggplot(mapping = aes(x = truth, y = mle)) + 
  geom_point(color="dark gray") +
  geom_point(mapping=aes(x=truth[cond2&!cond1], y=mle[cond2&!cond1]), color="#D55E00") +
...
grid.arrange(tveMle, tveApeglm, tveAsh, catPlot)
fig = arrangeGrob(tveMle, tveApeglm, tveAsh, catPlot)
ggsave("~/Desktop/ResultsLove/PaperPlots/realFigureColorblinded.png", fig)
```
...
topGenes = data.frame(absGoal = round(abs(goal), 2), totalCounts = totalCounts) %>%
  ...
  head(50)
topGenes
#write.csv(topGenes, "~/Desktop/topGenes.csv")
cat("quantiles of total counts\n")
quantile(totalCounts)
```

## Likelihood function range for case study
```{r likCase, echo=F}
set.seed(2395)
sub = subsets[2,]
pSub = p
