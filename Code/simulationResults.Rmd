...
tooLazyToComeUpWithAGoodTitleForFunction = function(p, maxDispersion=500) {
  cts=p$counts                           
...
param <- cbind(theta.hat, cts)
fit.mle <- apeglm(Y=ase.cts, x=X, log.lik=NULL, param=param, no.shrink=TRUE, log.link=FALSE, method="betabinCR")
#NOTE: took out 'coef=1' for last two


#fitting apeglme model
...
#saveRDS(object = CIgrid, file = "~/Desktop/CIgridNormal.rds")
CIgrid = readRDS("~/Desktop/CIgridNormal.rds")
```
...
MAplots = cowplot::plot_grid(MAplot1, MAplot2, align = "hv", ncol=1) + 
  ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/MAplotsNormal.eps") +
  ggsave("~/Desktop/ResultsLove/PaperPlots/MAplotsNormal.png")
plot(MAplots)
```
...
tveFilter = ggplot(mapping = aes(y=mle[filterRule3], x=truth[filterRule3])) +
  ...
  theme_minimal() +
  ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/tveAfterFilter.eps", width = 10, height = 4.5) +
  ggsave("~/Desktop/ResultsLove/PaperPlots/tveAfterFilter.png", width = 10, height = 4.5)
...
ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/normalFigure.eps", fig)
ggsave(filename ="~/Desktop/ResultsLove/PaperPlots/normalFigure.png", fig)
```
...
grid.arrange(tveMle, tveApeglm, tveAsh, catPlot)
fig = arrangeGrob(tveMle, tveApeglm, tveAsh, catPlot) 
ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/normalFigureColorblinded.eps", fig)
ggsave(filename ="~/Desktop/ResultsLove/PaperPlots/normalFigureColorblinded.png", fig)
```
...
#write.csv(map, "~/Desktop/studentTmap.csv")
#write.csv(mapsd, "~/Desktop/studentTsd.csv")
```
...
#CIgrid = ashci(ashresults)
#saveRDS(CIgrid, "~/Desktop/CIgridStudent")
CIgrid = readRDS("~/Desktop/CIgridStudent")
```
...
ggplot(mapping=aes(x=l$theta)) +
  ...
  theme_minimal() +
  ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/phi500Dens.eps", width = 10, height = 4.5) +
  ggsave("~/Desktop/ResultsLove/PaperPlots/phi500Dens.png", width = 10, height = 4.5)
...
ggplot(mapping = aes(x=beta)) + 
  ...
  coord_trans(y = "log2p") +
  theme_minimal() +
  ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/simulatedBeta.eps", width = 10, height = 4.5) +
  ggsave("~/Desktop/ResultsLove/PaperPlots/simulatedBeta.png", width = 10, height = 4.5)
```



## Filtering for Student's T Simulation
Again, the optimal filtering rule is now completely different.
...
The truth vs. estimate plots look different. We can see here that ash shrunk the gene that was truly largest to zero. In the previous analysis, when higher tolerance was used, ash did not shrink any truly large effects away from the trurth. 
```{r figStudent, echo=F}
cond1 = shrink>pmax(1,mle/4) 
cond2 = shrinkash>pmax(1,mle/4) 
cond3 = cond1 & cond2
...
grid.arrange(tveMle, tveApeglm, tveAsh, catPlot)
fig = arrangeGrob(tveMle, tveApeglm, tveAsh, catPlot) 
ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/studentTFigure.eps", fig) 
ggsave("~/Desktop/ResultsLove/PaperPlots/studentTFigure.png", fig)
```
...
grid.arrange(tveMle, tveApeglm, tveAsh, catPlot)
fig = arrangeGrob(tveMle, tveApeglm, tveAsh, catPlot) 
ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/studentTFigureColorblinded.eps", fig) 
ggsave("~/Desktop/ResultsLove/PaperPlots/studentTFigureColorblinded.png", fig) 
```
...
MAplots = cowplot::plot_grid(MAplot1, MAplot2, align = "hv", ncol=1) + 
  ggsave(device = "eps", filename ="~/Desktop/ResultsLove/PaperPlots/MAplotsStudentT.eps", width = 10, height = 4.5) +
  ggsave("~/Desktop/ResultsLove/PaperPlots/MAplotsStudentT.png")
plot(MAplots)
```



## Range of likelihood function for student's t
```{r likStudent, echo=F}
seedd = 2395
sub = c(2,3,5,6,14,15,20,21)
beta = rt(nrow(p$counts), df=3)/10
